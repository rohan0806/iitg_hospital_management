...
from rtc.models import CRequest,Medicine,FollowUp,Complaint,Doctor,Prescription,Reception,Patient,Internal_rim,Finance_accountant
import datetime
...
def rec_completed_list(request):
	...
	if len(Reception.objects.filter(username = username)) == 0:
		...
		now = datetime.datetime.now()
		rec_completed_list= CRequest.objects.filter(status='2', done = False ,appoint_date__year=now.year,appoint_date__month=now.month,appoint_date__day=now.day).order_by('appoint_date').reverse()
...
def print_prescription(request,tag):
	reg_no = tag
	prescription = Prescription.objects.filter(reg_no__reg_no=reg_no).order_by('prescription_time').reverse()[0]
	patient = Patient.objects.get(reg_no = reg_no)
	try: crequest = CRequest.objects.filter(reg_no = patient,status= '2',done= False)[0]
...
def followup(request,pres_id=0):
	...
	if len(Patient.objects.filter(username = username)) == 0:
		...
		if request.method == 'GET':
			...
			pres= Prescription.objects.filter(reg_no= regno1).order_by('prescription_time').reverse()
...
def in_rim(request):
	...
	if len(Patient.objects.filter(username = username)) == 0:
	    ...
	    if request.method == 'GET':
	        ...
	        t_amount1=0
	        t_amount1=t_amount1+int(exp_amount1)
	        exp_p2=request.POST['exp_p2']
	        exp_amount2=request.POST['exp_amount2']
	        if not exp_p2 == "":
	            exp_p1=exp_p1 + ":" + exp_p2
	            exp_amount1=exp_amount1 + ":" + exp_amount2
	            t_amount1=t_amount1+int(exp_amount2)
	        exp_p3=request.POST['exp_p3']
	        exp_amount3=request.POST['exp_amount3']
	        if not exp_p3 == "":
	            exp_p1=exp_p1 + ":" + exp_p3
	            exp_amount1=exp_amount1 + ":" + exp_amount3
	            t_amount1=t_amount1+int(exp_amount3)
	        exp_p4=request.POST['exp_p4']
	        exp_amount4=request.POST['exp_amount4']
	        if not exp_p4 == "":
	            exp_p1=exp_p1 + ":" + exp_p4
	            exp_amount1=exp_amount1 + ":" + exp_amount4
	            t_amount1=t_amount1+int(exp_amount4)
	        exp_p5=request.POST['exp_p5']
	        exp_amount5=request.POST['exp_amount5']
	        if not exp_p5 == "":
	            exp_p1=exp_p1 + ":" + exp_p5
	            exp_amount1=exp_amount1 + ":" + exp_amount5
	            t_amount1=t_amount1+int(exp_amount5)
	        exp_p6=request.POST['exp_p6']
	        exp_amount6=request.POST['exp_amount6']
	        if not exp_p6 == "":
	            exp_p1=exp_p1 + ":" + exp_p6
	            exp_amount1=exp_amount1 + ":" + exp_amount6
	            t_amount1=t_amount1+int(exp_amount6)
	        exp_p7=request.POST['exp_p7']
	        exp_amount7=request.POST['exp_amount7']
	        if not exp_p7 == "":
	            exp_p1=exp_p1 + ":" + exp_p7
	            exp_amount1=exp_amount1 + ":" + exp_amount7
	            t_amount1=t_amount1+int(exp_amount7)
	        exp_p8=request.POST['exp_p8']
	        exp_amount8=request.POST['exp_amount8']
	        if not exp_p8 == "":
	            exp_p1=exp_p1 + ":" + exp_p8
	            exp_amount1=exp_amount1 + ":" + exp_amount8
	            t_amount1=t_amount1+int(exp_amount8)
...
def ins_search(request):
	...
	if len(Finance_accountant.objects.filter(username = username)) == 0:
		...
		if request.method=='GET':
			return render(request,'rtc/ins_search.html')
		elif request.method=='POST':
			user_name=request.POST['user_name']
			rim_objects=Internal_rim.objects.filter(username=user_name)
			temp="mayank"
			for obj in rim_objects:
				exp_part=obj.exp_p
				temp=exp_part
			...
			for i in range(num):
				...
				list3.append(dic)
			context={'rim_objects':rim_objects,'list3':list3,'num':num}
			return render(request,'rtc/ins_search.html',context)
#############################################################################################################
#############################################################################################################
#############################################################################################################
